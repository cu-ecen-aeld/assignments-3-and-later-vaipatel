...
{
    char* actualUsernameInCfg = malloc_username_from_conf_file();
    const char* expectedUsername = my_username();
    TEST_ASSERT_EQUAL_STRING_MESSAGE(expectedUsername, actualUsernameInCfg, "Username in config does not match the expected username");
    free(actualUsernameInCfg);
}
